<head>
<style type="text/css">
body{background-image:url("http://descargarfondosdepantalla.com/wp-content/uploads/2016/04/fondos-de-pantalla-verdes-simple.jpg")}
</style>
</head>
<body> 
   <tr height="50px">
      <td colspan="2px
	  <center>
         <table title="Banner" border="0">
            <tr>
               <td><img src="https://www.upalt.edu.mx/images/Logo_UPALT4.jpg"></td>
			   <center>
			   <td><br><br><h1> UNIVERSIDAD POLITECNICA DE ALTAMIRA</h1></td>
			   </center>
            </tr>
         </table>
		 </center>
      </td>
   </tr>
   <tr height="200px" style="text-align:center;">
      <td>
	  
         <table title="Menu" border="0" width="200px">
            <tr><td style="text-align:center;">Menu</td></tr>
			<tr><td><li><a href=ejemplo.html </a>Inicio</li></td></tr>
            <tr><td><li><a href=carreras.html </a>Carreras</li></td></tr>
            <tr><td><li><a href=instalaciones.php </a>Instalaciones</li></td></tr>
            <tr><td><li><a href=contactos.php </a>Contactos</li></td></tr>
         </table>
      </td>
      <td>
	  <center>
         
           <tr><h4>Contactos</h4></tr>
			<tr><td><br><br>Direccion:<tr></td></tr>
			<td><br>Nuevo Libramiento Altamira KM 3 Santa Amalia, Altamira, Tamps. C.P.</td>
			<tr><td><br><br>Telefono:<tr></td></tr>
			<td>833-304-04-74</td>
			<tr><td><br><br>Email:<tr></td></tr>
			<td>contacto@upalt.edu.mx</td>
         
	   </center>
     </td>
  </tr>
